...
contract RPS {
	...
	modifier validChoice(string choice){
		require(keccak256(choice) == keccak256("Ethereum") || keccak256(choice) == keccak256("SEC") || keccak256(choice) == keccak256("Bitcoin"));
		_;
	}
	...
	function open(string choice,string randStr) public isRegistered bothLocked validChoice(choice) returns (bool) {
		...
		if(msg.sender ==user1){
			if(tempHash==hash1){
				if(keccak256(choice)==keccak256("Ethereum")){
					claimed1 = 1;
					if(firstRevealTime==0)
						firstRevealTime = now;
					return true;
				}
				if(keccak256(choice)==keccak256("SEC")){
					claimed1 = 2;
					if(firstRevealTime==0)
						firstRevealTime = now;
					return true;
				}
				if(keccak256(choice)==keccak256("Bitcoin")){
					claimed1 = 3;
					if(firstRevealTime==0)
						firstRevealTime = now;
					return true;
